  ...
  return (
    <div className="space-y-2 animate-slideInUp">
      <div className="text-center">
        <div className="w-12 h-12 bg-gradient-to-br from-green-500 to-teal-600 rounded-full mx-auto mb-2 flex items-center justify-center shadow-lg relative group">
          <span className="text-xl">🏘️</span>
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10">
            Tell us how you plan to use this property
          </div>
        </div>
        <h2 className="text-lg font-bold text-gray-800 bg-gradient-to-r from-green-600 to-teal-600 bg-clip-text text-transparent">
          Property Usage
        </h2>
      </div>
      ...
      <div className="text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {propertyUsages.map((usage, index) => (
            <button
              key={usage.value}
              onClick={() => handleSelection(usage.value)}
            ...
            >
              <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-all duration-300 ${
                ...
                <span className="text-4xl group-hover:scale-110 transition-transform">{usage.icon}</span>
              </div>

              <div className="mb-4">
                <span className="font-bold text-xl block mb-2">{usage.label}</span>
                <span className={`text-sm ${formData.loanDetails.propertyUsage === usage.value ? 'text-gray-700' : 'text-gray-400'}`}>
                  {usage.description}
                </span>
              </div>
      ...
      <div className="flex justify-between pt-8">
        ...
        {formData.loanDetails.propertyUsage && (
          <button
            onClick={() => nextStep()}
            className="bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 hover:scale-105 hover:shadow-2xl flex-1 ml-4 animate-glow"
          >
        ...
        <div className="text-center mt-12">
          <div className="flex items-center justify-center space-x-3 text-gray-600 text-lg">
            ...
            <span className="font-medium">Step 3 of 11</span>
            <div className="text-sm text-gray-900">• You're doing great!</div>
          </div>
        </div>
    </div>
  );
